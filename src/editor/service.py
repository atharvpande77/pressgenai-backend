...
async def get_articles_by_publish_status(session: AsyncSession, editor_status: str, limit: int = 10, offset: int = 0):
    try:
        res = await session.execute(select(GeneratedUserStories.id, GeneratedUserStories.title, GeneratedUserStories.snippet, GeneratedUserStories.full_text, GeneratedUserStories.created_at).join(UserStories, onclause=UserStories.id == GeneratedUserStories.user_story_id).filter(and_(UserStories.publish_status == editor_status, UserStories.status == UserStoryStatus.SUBMITTED)).order_by(UserStories.created_at.desc()).limit(limit).offset(offset))
        articles = res.all()
        if not articles:
...
async def update_article_db(session: AsyncSession, article_id: str, payload: EditArticleSchema):
    stmt = update(GeneratedUserStories).where(GeneratedUserStories.id == article_id).values(payload.model_dump()).returning(GeneratedUserStories)
...
async def publish_article_db(session: AsyncSession, article_db: GeneratedUserStories):
    publish_status = await set_publish_status(session, article_db.user_story_id, UserStoryPublishStatus.PUBLISHED)
    if not publish_status:
...
async def reject_article_db(session: AsyncSession, article_db: GeneratedUserStories, reason: str):
